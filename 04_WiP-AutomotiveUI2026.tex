...
\begin{figure}[h]
  ...
  \fbox{\parbox{0.92\columnwidth}{\centering\vspace{2em}
    \textit{[Figure 1: Mediation architecture — Ontology Language + Schema
    Profile is the authoritative reference (provides schema to Trust Policy,
    contract to Translation Layer); runtime flow is
    Agents/ADAS → Trust Policy → Translation → Runtime → Renderer ↔ Occupant;
    renderer capabilities feed back into Translation; Context Policy modulates
    Translation and Runtime.
    Insert PDF export of Mermaid diagram here.]}
  \vspace{2em}}}
  \caption{Mediation architecture. The Ontology Language + Schema Profile is
    the authoritative reference --- Trust Policy validates against it,
    Translation Layer interprets by it. Context Policy modulates Translation
    and Runtime; renderer capabilities feed back into Translation.}
