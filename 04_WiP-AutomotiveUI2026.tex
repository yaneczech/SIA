...
\begin{figure}[h]
  ...
  \fbox{\parbox{0.92\columnwidth}{\centering\vspace{2em}
    \textit{[Figure 1: Mediation architecture — Trust Policy gates entry;
    Ontology Language + Schema Profile defines meaning; Translation Layer
    selects modalities; Runtime coordinates; Renderer Layer faces occupant.
    Context Policy modulates Translation and Runtime.
    Insert PDF export of Mermaid diagram here.]}
  \vspace{2em}}}
  \caption{Mediation architecture. Trust Policy gates all incoming nodes;
    the Ontology Language + Schema Profile is the canonical source of meaning;
    Context Policy modulates Translation and Runtime throughout.}
...
semantic stream and face the occupant directly. Each renderer declares
measurable capabilities; the Translation Layer selects among them. All nodes
reaching the Renderer Layer have already been verified and coordinated.

\paragraph{Trust Policy.}
A gate at the entry point of SIA: all nodes emitted by agents, services, or
ADAS systems pass through trust verification before entering the Ontology
layer. Nodes that fail are rejected and logged; they never reach the semantic
pipeline.

\paragraph{Context Policy.}
Supplies a continuously updated context vector that modulates both the
Translation Layer (modality selection) and the Interaction Coordination
Runtime (conflict resolution, acknowledgement timeouts).
