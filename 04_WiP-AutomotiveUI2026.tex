...
absent layer of architecture (Figure~\ref{fig:complexity}).

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{figures/No_logo/fig1-complexity-comparison.png}
  \caption{Where the cross-cutting interaction logic lives, before vs. after
    SIA. Without SIA, every emitter--renderer pair carries its own
    implementation of trust, context, capability, accessibility, fallback,
    and audit logic --- a matrix of $N \times M$ duplications that diverges
    over time. With SIA, this cross-cutting logic exists once at the
    mediation boundary; renderers become thin stateless consumers and adding
    new emitters or renderers is linear, not multiplicative.}
...
addresses (Figure~\ref{fig:stack}).

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{figures/No_logo/fig2-stack-position.png}
  \caption{Position of the proposed Semantic Interaction Architecture
    relative to existing SDV layers. SIA sits above data and service
    abstractions (COVESA VSS, Kuksa, uProtocol) and below physical HMI
    surfaces (HUD, cluster, IVI, voice, haptic, AR).}
  \label{fig:stack}
\end{figure}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{figures/No_logo/fig3-mediation-architecture.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{figures/No_logo/fig4-node-taxonomy.png}
...
\begin{figure}[h]
  ...
  \fbox{\parbox{0.92\columnwidth}{\centering\vspace{2em}
    \textit{[Figure 5: Sequence diagram --- ADAS emits instance + attestation to
