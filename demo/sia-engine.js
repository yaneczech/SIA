  ...
  return {
    ontology: {
      ...
      trace: {
        ...
        declaration: node,
        attention_estimate: {
          metric: 'predicted_total_glance_time',
          value_ms: node.attention.glanceTimeMs,
          applicable_renderer_classes: ['cluster', 'ivi'],
    ...
    translation: {
      ...
      explanation: 'Translation combines immutable node meaning with live context, a context-scaled attention estimate, and declared renderer capabilities. The demo values are illustrative fixtures; a deployment must calibrate its own empirical model.',
      ...
      rule: { title: deferred ? 'Retention policy' : decision.attention ? 'Capability negotiation' : 'Context decision', items: decision.attention ? [`illustrative base ${decision.attention.base} ms × modifier ${decision.attention.modifier} = ${decision.attention.effective} ms`, 'provenance: illustrative_demo_fixture · unvalidated', node.type === 'Alert' ? 'Critical Alert → safety-certified/eyes-free surfaces only' : 'Renderer eligible only if effective estimate ≤ its budget', 'Use declared renderer order'] : deferred ? [`Disposition: ${decision.retention.disposition}`, `TTL: ${decision.retention.ttlMs} ms`, `Re-evaluate on: ${decision.retention.reevaluateOn.join(', ')}`] : [`Applicability: ${node.contextPolicy.applicability}`, `Blocked disposition: ${node.contextPolicy.onBlocked.disposition}`, `Decision: ${decision.outcome}`] },
